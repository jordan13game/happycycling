...
void rpc_connecter_t::remote_open(bool use_monitor_)
{
    ...
    logdebug((RPC_CONNECTER, "%s", m_socket.remote_endpoint().address().to_string().c_str()));
...
void rpc_connecter_t::sync_write_i(string& data_)
{
    ...
    logdebug((RPC_CONNECTER, "%02X %02X %02X %02X %02X %02X %02X %02X",(uint8_t)data_[0],(uint8_t)data_[1],(uint8_t)data_[2],(uint8_t)data_[3],(uint8_t)data_[4],(uint8_t)data_[5],(uint8_t)data_[6],(uint8_t)data_[7]));
    /*
