...
#define LUA_REGISTER_BEGIN(cpp_register) \
    static void cpp_register(ys_lua_t& handler_) {\
...
#define DEFINE_LUA_EXT_BEGIN(x) extern "C" { int LUA_API luaopen_##x (lua_State* ls_) { ys_lua_t ys_lua(ls_);
#define DEFINE_LUA_INTERFACE(x)  ys_lua.multi_register(x);
